...
which.max(fib_vec) #return the indices where the maximum value exists
which.min(fib_vec) #return the indices where  the minimum value exists
...
all(catalan_vec==1) #all of the values equals 1
...
#try sorting the repeats backward
sort(rep(catalan_vec,2), decreasing = TRUE)
