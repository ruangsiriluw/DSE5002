...
#making new columns "!religion" means to keep the religion column
head(relig_long)
...
wide_fish_na <- fish_encounters %>%
  pivot_wider(names_from = station, values_from = seen) %>%
  dplyr::mutate_at(c(8:12), ~replace_na(.,0))  #replace NA with "0" for all values in df
#only replace at column 8-12
