...
stringr::str_split(sales$Customer.Name, ' ',simplify = T) 
   ## simplify = F Return a character vector, if T return a matrix
...
left_df <- df1 %>%   #take keys of df2 join with df1, at left
  left_join(df2,by='CustomerId')
left_df

right_df <- df1 %>%  #Take keys of df2 join with df1, at right
